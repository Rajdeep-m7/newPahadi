  ...
  return (
    <div className="p-8 space-y-8 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 bg-surface p-4 rounded-xl border border-border">
        <div className="flex items-center gap-1 bg-background p-1 rounded-lg overflow-x-auto w-full xl:w-auto">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
          ...
          <div className="relative">
            <button
              ...
              className={`p-2 border border-border rounded-lg hover:bg-background transition-colors ${showFilterModal ? 'bg-background text-brand-dark border-brand' : 'text-muted'}`}
            >
              <Filter size={20} />
            </button>
            {showFilterModal && (
              <div ref={filterModalRef} className="absolute top-full right-0 mt-2 w-[320px] bg-surface border border-border rounded-xl shadow-xl z-50 p-6 animate-in fade-in zoom-in-95 duration-200">
                <div className="flex justify-between items-center mb-6">
                  <h4 className="font-bold text-primary">Filter Orders</h4>
                  <button onClick={() => setShowFilterModal(false)} className="text-muted hover:text-primary"><X size={18} /></button>
                </div>

                <div className="space-y-5">
                  <div>
                    <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Brand</label>
                    <select
                      value={selectedBrand}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-background border border-border rounded-lg p-2 pl-9 text-sm focus:outline-none focus:border-brand"
                      />
                    ...
                    {searchCategory && (
                      <div className="mt-2 max-h-32 overflow-y-auto border border-border rounded-lg bg-background">
                        {filteredCategories.map(cat => (
                          <button
                            key={cat}
                            onClick={() => { setSelectedCategory(cat); setSearchCategory(''); }}
                            className="w-full text-left px-3 py-2 text-sm hover:bg-surface transition-colors flex items-center justify-between"
                          >
                            {cat}
                            {selectedCategory === cat && <Check size={14} className="text-brand-dark" />}
                          </button>
                        ))}
                      </div>
                    )}
                    {selectedCategory && !searchCategory && (
                      <div className="mt-2 flex items-center justify-between bg-brand/10 text-brand-dark px-3 py-1.5 rounded-lg text-sm">
                  ...
                  <div>
                    <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Date Range</label>
                    <div className="flex items-center gap-2">
                      <input type="date" className="flex-1 bg-background border border-border rounded-lg p-2 text-xs focus:outline-none focus:border-brand" />
                ...
                <div className="mt-8 flex gap-3">
                  <button className="flex-1 py-2 rounded-lg text-sm font-medium border border-border hover:bg-background transition-colors">Reset</button>
                  <button className="flex-1 py-2 rounded-lg text-sm font-medium bg-brand text-white hover:bg-brand-dark transition-colors">Apply</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="relative">
            <button
              ...
              className={`p-2 border border-border rounded-lg hover:bg-background transition-colors ${showSortModal ? 'bg-background text-brand-dark border-brand' : 'text-muted'}`}
            >
            ...
            {showSortModal && (
              <div ref={sortModalRef} className="absolute top-full right-0 mt-2 w-48 bg-surface border border-border rounded-xl shadow-xl z-50 py-2 animate-in fade-in zoom-in-95 duration-100">
                ...
                {[
                  ...
                  <button key={option} className="w-full text-left px-4 py-2.5 text-sm text-primary hover:bg-background transition-colors">
                    {option}
                  </button>
                ))}
              </div>
                  ...
                  <div className="px-8 py-4 flex items-center justify-between text-sm">
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex items-center gap-1 ml-4">
                        <span className="font-bold text-primary">Date of Order:</span>
