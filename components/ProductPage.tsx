...
const ProductPage = ({ product, variant }: ProductPageProps) => {
  ...
  const categoryName =
    productDetails?.categoryId?.name || product?.categoryName || "Premium Jewellery";
  const returnPolicy =
  ...
  const discount =
    mrp > price ? Math.round(((mrp - price) / mrp) * 100) : product?.discount || 0;
  ...
  const productImages = useMemo(() => {
    ...
    const images = variantImages.length ? variantImages : [product?.image || fallbackImage];
    ...
    <section className="py-3">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[520px_1fr]">
        ...
        <div className="pb-28 lg:pb-0">
          ...
          <p className="mt-6 text-sm leading-7 text-gray-600 dangrously-set-inner-html">
            {description}
          </p>
          ...
          <div className="mt-8 space-y-4 rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
            {(productDetails?.specs?.length
              ...
              : [
                  { key: "Brand", value: product?.brand || "Pahadi Collections" },
