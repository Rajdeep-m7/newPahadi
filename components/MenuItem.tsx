...
export default function MenuItem({ title, image }: MenuItemProps) {
  return (
    <Link href={`/category/${title.toLowerCase().replace(/\s+/g, '-')}`} className="group relative flex items-center gap-2 px-3 py-1 pb-4 cursor-pointer">
      <Image
        height={20}
        width={20}
        src={image}
